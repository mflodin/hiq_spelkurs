...
Ball.controller = function(params) --optional parameter
  ...
  return function(actor, deltaTime)  
    ...
    if not(actor.direction.y == change.direction.y) then
      actor.speed = actor.speed + params.speedIncrease
    end
...
Ball.collision = {}
Ball.collision.test = {}
Ball.collision.test.paddle = function(ball, paddle)  
  return Collision.boxVsBox(ball.box, paddle.box)  
end
...
Ball.collision.paddle = function(ball, paddle)
  ...
  elseif overlap.x < overlap.y then -- bounce horizontally
    ...
    ball.speed = ball.speed + Ball.speedIncrease   
  end  
end
...
Ball.screenBoundaryCollision = function(ball, onHit) 
  ...
  if ball.position.x + ball.size.w2 > love.graphics.getWidth() + ball.size.w then
    change.direction.x = -ball.direction.x
    local distanceFromCollision = (love.graphics.getWidth() + ball.size.w) - (ball.position.x + ball.size.w2)      
  ...
  --Collision vs left
  if ball.position.x - ball.size.w2 < -ball.size.w then
    change.direction.x = -ball.direction.x      
    local distanceFromCollision = (ball.position.x - ball.size.w2) - -ball.size.w      
    change.position.y = -distanceFromCollision
